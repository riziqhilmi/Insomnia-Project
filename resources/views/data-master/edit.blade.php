...
<div class="max-w-4xl mx-auto mt-10 bg-blue-900 text-white p-8 rounded-xl shadow-md">
    ...
    <form action="{{ route('data-master.update', $dataMahasiswa) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        @foreach ([
            ...
            <div>
                ...
                <input type="text" name="{{ $field }}" id="{{ $field }}" value="{{ old($field, $dataMahasiswa->$field) }}"
                       class="mt-1 w-full rounded-md bg-blue-800 border border-blue-700 p-2 text-white focus:ring focus:ring-blue-300"
                       required>
            </div>
        @endforeach
